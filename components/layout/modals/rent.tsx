...
export default function RentModal() {
  ...
  let bodyContent = (
    <div className='flex flex-col gap-8'>
      ...
      gap-3 max-h-[50vh] overflow-y-auto'>
        {categories.map(item => <div key={item.label} className='grid-span-1'>
          <CategoryInput label={item.label} 
  ...
  if(step === STEPS.LOCATION) {
    bodyContent = (<div className='flex flex-col gap-8'>
      <Heading
        title="Where is your place locatioed?"
  ...
  if(step === STEPS.IMAGES) {
    bodyContent = (<div className='flex flex-col gap-8'>
      <Heading
        title="Add a photo a your place"
  ...
  if(step === STEPS.PRICE) {
    bodyContent = (<div className='flex flex-col gap-8'>
      ...
      />
      <Input id="price" label="Price" disabled={isLoading} register={register} type="number" errors={errors} required/>
    </div>)
  }
  return (
    <Modal
